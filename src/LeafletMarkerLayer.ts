...
import { ILeafletMarkerOption, LeafletMarker } from './LeafletMarker'

export class LeafletMarkerLayer {
  private _markLayerIdMap: Map<string, LeafletMarker> = new Map()
      ...
      (() => {
        for (const [, gLyr] of this._markerClusterGroupIdMap) {
          for (const childLyrOfGLyr of gLyr.getLayers()) {
            if (childLyrOfGLyr instanceof LeafletMarker && childLyrOfGLyr.id === id) {
  ...
  createMarker(option: ILeafletMarkerOption) {
    const markerLyr = new LeafletMarker({
      icon: L.icon({
        iconSize: [25, 41],
        iconAnchor: [10, 41],
        popupAnchor: [2, -40],
    ...
    this._markLayerIdMap.set(option.id, markerLyr)
    return markerLyr
  }
